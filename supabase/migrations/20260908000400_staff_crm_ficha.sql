-- ST-3a.4 — ficha de cliente do CRM (leitura consolidada, curada por papel).
--
-- staff_crm_ficha(p_ref, p_hist_cursor)
--   p_ref: 'conta:<uuid>' | 'crm:<bigint>'
--   p_hist_cursor null  → ficha completa + 1ª página do histórico (20)
--   p_hist_cursor set   → SÓ { historico, historico_next } (paginação)
--
-- Autorização (não vaza posse — NOT_FOUND):
--   admin / vendas → qualquer ref
--   barbeiro       → (a carteira dele) OU (atendeu essa conta)
--
-- Curadoria (decisões fechadas):
--   • telefone / e-mail SEMPRE mascarados (D-ST3-10 / D-ST3-6)
--   • vendas: sem idade, sem `appointments.notes` (D-ST3-6)
--   • SEM bloco financeiro/gasto (D-ST3-4), SEM plano (D-ST3-7)
--   • NUNCA `sales.*` / `sale_payments.*` / `fiado_*` / `referral_code` /
--     `appointments.rating*` / `auth.users`
--   • barbeiro: histórico/frequência só dos atendimentos DELE
--
-- Rollback: drop function public.staff_crm_ficha(text, text);
...
begin
  ...
  -- keyset por dia. Se a 20ª e a 21ª linha caírem no MESMO dia, a próxima
  -- página começa antes desse dia (raras 2+ visitas no mesmo dia podem ser
  -- puladas — inofensivo p/ um histórico de CRM).
  with h as (
  ...
  -- barbeiro mais recorrente: só faz sentido cross-barbeiro (admin/vendas).
  -- barbeiro vê só a si → null.
