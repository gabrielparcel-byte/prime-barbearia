...
- [x] Modo offline básico (ver agenda mesmo sem internet) ✅ *(2026-07-13)* — `sw.js` ganhou cache do app shell (`index_9.html`, `manifest.json`, ícones), estratégia network-first same-origin com fallback pro cache quando offline (fontes/CDN do Supabase seguem direto pra rede, sem cache, por serem cross-origin). Agenda do barbeiro (`baFetchAgendaAppointments`/`baFetchPendingAppointments`) passa a espelhar a última busca bem-sucedida em `localStorage` (chaveado por barbeiro) e cai pra esse cache quando a rede falha. Banner "Sem conexão — mostrando agenda salva às HH:MM" aparece na tela de Agenda; some e recarrega os dados sozinho ao voltar a conexão (`window.addEventListener('online'/'offline')`). Fora do escopo (deliberado): fila de ações offline — aceitar/iniciar atendimento continuam exigindo rede.
- [x] Compartilhar conquista nas redes sociais (story template) ✅ *(2026-07-13)* — botão "Compartilhar" na celebração de desbloqueio e no detalhe do sticker; gera imagem formato story (1080x1920, canvas) com a medalha, nome, descrição e marca; usa a Web Share API nativa (compartilha direto pro Instagram/WhatsApp) com fallback de download
